...
# checkout before building the macOS app (scripts/macos-release.sh runs it for
# you). When external/Sparkle is absent the CMake auto-update block compiles the
# bridge to a no-op and the app still builds.
#
# The framework ships pre-signed by the Sparkle project; macos-release.sh
# re-signs it (and its nested XPC services / Autoupdate / Updater.app) with our
# Developer ID before notarization. bin/generate_keys and bin/sign_update are
# used for the EdDSA update-signing key and per-release appcast signatures
