#!/usr/bin/env bash
# Add a release to the Sparkle appcast (docs/appcast.xml — macOS feed).
#
# Everything except the EdDSA signature and the release notes is derived:
#   version  → CMakeLists.txt project(VERSION)  (or arg 1)
#   tag      → v<version>
#   DMG      → release/minRender-<version>-macOS.dmg (or arg 2)
#   URL      → github.com/<repo>/releases/download/v<version>/minRender-<version>-macOS.dmg
#   sig+len  → external/Sparkle/bin/sign_update <dmg>  (uses the Keychain key)
#   date     → date -R
#
# Run AFTER scripts/macos-release.sh has produced the notarized DMG and you've
# created the GitHub release tag v<version> with that DMG uploaded. Then review
# + commit docs/appcast.xml to publish at minrender.com/appcast.xml.
#
# Usage:
...
DMG="${DMG:-$REPO_ROOT/release/minRender-$VERSION-macOS.dmg}"
...
PUBDATE="$(date -R)"
URL="https://github.com/$REPO/releases/download/$TAG/minRender-$VERSION-macOS.dmg"
